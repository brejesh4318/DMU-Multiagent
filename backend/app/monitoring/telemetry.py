...
def track_llm_call(response, model: str, t0: float) -> TokenUsage:
    """Extract token usage from a Groq ChatCompletion (LangChain AIMessage) response.

    Handles both shapes LangChain exposes:
    ...
    Either may be a dict or an attribute-bearing object, so we coerce to a dict first.
    """
    def _to_dict(u) -> dict:
        if u is None:
            return {}
        if isinstance(u, dict):
            return u
        # attribute-style object (e.g. pydantic UsageMetadata)
        return {
            k: getattr(u, k)
    ...
    usage = _to_dict(getattr(response, "usage_metadata", None))
    if not usage:
        usage = _to_dict(getattr(response, "response_metadata", {}).get("token_usage"))

    # input_tokens (usage_metadata) | prompt_tokens (token_usage)
    ...
    total_t  = int(usage.get("total_tokens", 0) or 0) or (input_t + output_t)
